...
export function HrAiAssistantView() {
  const { currentUser, isolatedJobs, isolatedCandidates, companies } = useAts();
  ...
  >([
    {
      role: "assistant",
      content: `Hello ${currentUser.name}! I am your Recruiter AI Copilot for ${currentCompany?.name || "your company"}. I can analyze your hiring pipeline, recommend requisition adjustments, benchmark PKR salary offers against Pakistani market rates, and formulate role-specific technical screening rubrics. How can I assist your team today?`,
      suggestions: [
        "How can I improve candidate pipeline velocity?",
        "Benchmark salary for a Senior Frontend Developer in Lahore",
        "What are the most common reasons candidates drop off?",
        "Suggest a screening rubric for Backend Engineers",
      ],
    },
  ]);
  ...
  const handleSend = (queryText?: string) => {
    ...
    const response = aiServices.hrAssistant(
      text,
      {
        name: currentUser.name,
        company: currentCompany?.name || currentUser.company || "Company",
      },
      {
    ...
    const formattedContent = `### ${response.title}\n\n${response.response}\n\n**Action Items for Recruiter:**\n${response.suggestedActions.map((a: string) => `• ${a}`).join("\n")}`;
    ...
    setMessages((prev) => [
      ...
      userMsg,
      {
        ...
        suggestions: [
          "Explain more about Pakistani market hiring benchmarks",
          "Help me optimize our job requisition requirements",
          "How can we attract more senior candidates?",
        ],
      },
    ]);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border bg-primary/5 px-3 py-1 text-xs font-semibold text-primary">
              ...
              <span>AI Hiring Intelligence & Talent Copilot</span>
            </div>
            ...
            <p className="mt-1 text-xs sm:text-sm text-muted-foreground">
              Intelligent recommendations on candidate screening, compensation calibration in PKR, and requisition health.
            </p>
          </div>

          <Badge variant="outline" className="text-xs px-3 py-1 font-mono">
            {isolatedCandidates.length} Active Candidates Evaluated
          </Badge>
        </div>
      </div>
      ...
      <div className="rounded-2xl border bg-card flex flex-col h-[560px] overflow-hidden shadow-xs">
        ...
        >
          <Input
            ...
            placeholder="Ask Recruiter Copilot about pipeline velocity, PKR salary benchmarks, or screening rubrics..."
